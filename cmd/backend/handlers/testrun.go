...
// testRunWithVersion wraps a TestRun with the procedure version it was created against.
...
func (h *TestRunHandler) List(w http.ResponseWriter, r *http.Request) {
	...
	// Enrich each run with its procedure version.
	enriched := make([]testRunWithVersion, len(runs))
	for i, run := range runs {
		enriched[i] = testRunWithVersion{
			TestRun:          *run,
			ProcedureVersion: versionMap[run.TestProcedureID],
		}
	}

	respondJSON(w, http.StatusOK, NewPaginatedResponse(enriched, total, limit, offset))
}
